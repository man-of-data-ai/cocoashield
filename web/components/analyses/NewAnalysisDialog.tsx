...
export default function NewAnalysisDialog({ open, onClose, parcelId, parcelName, onAnalysisCreated }: Props) {
  ...
  useEffect(()=>{
    if (!open) return;
    Promise.allSettled([configurationService.listDroneProfiles(true), missionService.listMissions()]).then(([profilesResult,missionsResult])=>{
  ...
  async function submit(){
    ...
    if(missionMode === "existing" && !missionId){setError("Sélectionnez la mission correspondant à cette collecte terrain.");return;}
    if(missionMode === "new" && !missionName.trim()){setError("Donnez un nom à la mission de collecte.");return;}
  ...
  return <Dialog open={open} onClose={close} title={parcelName ? `Nouvelle analyse · ${parcelName}` : "Nouvelle analyse"}><div className="space-y-6">
    ...
    <div><label className="mb-1.5 block text-xs font-bold text-slate-600">Profil de drone / détection</label><ModernSelect value={profileId} onChange={setProfileId} options={[{value:"",label:"Détection standard"},...profiles.map((p)=>({value:p.profileId,label:p.profileId,description:`${p.manufacturer} ${p.model}`}))]}/></div>
