...
function Fit({ parcel }: { parcel: Parcel }) {
  const map = useMap();
  useEffect(() => {
    const b = parcel.boundary.coordinates[0].map(([lng, lat]) => [
      lat,
      lng,
    ]) as LatLngBoundsExpression;
    map.fitBounds(b, { padding: [30, 30], maxZoom: 18, animate: false });
  }, [map, parcel]);
  return null;
}
...
export default function AnalysisImageMap({
  ...
  const zones = useMemo(
    ...
    [parcel, analysis, thresholds],
  );
  const located = analysis.images.filter(
    (image) =>
      image.latitude !== null &&
      image.longitude !== null &&
      image.geolocationQuality === "precise",
  );
  return (
      ...
      >
        ...
        {located.map((image) => (
          <CircleMarker
            ...
            radius={image.id === selectedImageId ? 5 : 3}
            pathOptions={{
              color: "#FFFFFF",
              weight: 1.5,
              fillColor: "#334155",
            ...
            eventHandlers={{ click: () => onSelectImage(image.id) }}
          >
            <Tooltip>Observation géolocalisée</Tooltip>
          </CircleMarker>
        ))}
        <Fit parcel={parcel} />
      </MapContainer>
