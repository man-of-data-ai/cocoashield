...
function computeBounds(parcels: Parcel[]): LatLngBoundsExpression | undefined {
  const points: [number, number][] = [];
  for (const parcel of parcels)
    for (const [lng, lat] of parcel.boundary.coordinates[0])
      points.push([lat, lng]);
...
export default function CampaignComparisonMap({
  ...
  const bounds = useMemo(
    () => computeBounds(referenceParcels),
    [referenceParcels],
  );
  const snapshotByParcel = useMemo(
    () => new Map(snapshot.parcels.map((item) => [item.parcel.id, item])),
    [snapshot.parcels],
  );
  const { thresholds } = useSeverityThresholds();
  const zones = useMemo(
    () =>
      referenceParcels.flatMap((parcel) =>
        (parcel.analyses ?? [])
          .filter((analysis) => analysis.missionId === snapshot.mission.id)
          .flatMap((analysis) =>
            riskZonesFromAnalysis(parcel, analysis, thresholds),
          ),
      ),
    [referenceParcels, snapshot.mission.id, thresholds],
  );
  return (
    <div className="relative h-[430px] min-h-[360px] overflow-hidden rounded-xl bg-slate-100 lg:h-[500px]">
      <MapContainer
        center={DEFAULT_MAP_CENTER}
        zoom={DEFAULT_MAP_ZOOM}
        bounds={viewport ? undefined : bounds}
      ...
      >
        ...
        />
        {referenceParcels.map((parcel) => {
          const history = snapshotByParcel.get(parcel.id);
          return (
            <Polygon
              key={parcel.id}
              positions={ringToLatLngs(parcel.boundary.coordinates[0])}
              pathOptions={{
                color: history ? "#64748B" : "#94A3B8",
                weight: 2,
                fillColor: "#E2E8F0",
                fillOpacity: history ? 0.14 : 0.05,
                dashArray: history ? undefined : "5 5",
              }}
            >
              <Tooltip sticky>
                <div className="space-y-0.5">
                  ...
                  <div>
                    {history
                      ? `Infection ${(history.infectionRate * 100).toFixed(1)} %`
                      : "Aucun relevé pour cette campagne"}
                  </div>
                </div>
              </Tooltip>
            </Polygon>
        ...
        <SeverityHeatmapLayers
          zones={zones}
          activeLevels={activeSeverityLevels}
          onSelectZone={onSelectRiskZone}
        />
