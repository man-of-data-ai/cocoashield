      ...
      return <Fragment key={level}>
        ...
        {levelZones.filter((zone) => zone.geometry).map((zone) => (
          <Polygon
            ...
            pathOptions={{ color: SEVERITY_COLORS[level], fillColor: SEVERITY_COLORS[level], fillOpacity: 0.42, weight: 1.5 }}
            eventHandlers={{ click: () => onSelectZone?.(zone) }}
          >
            <Tooltip>{zone.parcelName} · {SEVERITY_LABELS[level]}</Tooltip>
          </Polygon>
        ))}
        {levelZones.filter((zone) => !zone.geometry).map((zone) => (
          <CircleMarker
