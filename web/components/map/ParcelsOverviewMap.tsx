...
function FitParcelsBounds({
  ...
  useEffect(() => {
    if (bounds)
      map.fitBounds(bounds, { padding: [36, 36], maxZoom: 17, animate: false });
  }, [map, bounds]);
  return null;
}

export default function ParcelsOverviewMap({
  parcels,
  riskZones,
    ...
    >
      ...
      {layers.heatmap && (
        <SeverityHeatmapLayers
          ...
          onSelectZone={(zone) => {
            onSelectParcel(zone.parcelId);
            onSelectRiskZone?.(zone);
          }}
        />
      )}
    </MapContainer>
