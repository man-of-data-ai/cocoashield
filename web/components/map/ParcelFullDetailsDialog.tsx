...
export default function ParcelFullDetailsDialog({
  ...
  const images = useMemo(
    () => analyses.flatMap((analysis) => analysis.images ?? []),
    [analyses],
  );

  // Réinitialisation à l'ouverture ou au changement de parcelle, ajustée
  // pendant le rendu plutôt que dans un effet : un effet qui recopie des
  // props dans un état déclenche un rendu en cascade et affiche brièvement
  // les valeurs de la parcelle précédente.
  const contentKey = open
    ? `${parcel?.id ?? ""}:${latestAnalysis?.id ?? ""}`
    : null;
  const [syncedKey, setSyncedKey] = useState<string | null>(null);
  if (contentKey !== null && contentKey !== syncedKey) {
    setSyncedKey(contentKey);
    setActiveTab("summary");
    setVerification(parcel?.terrainVerificationStatus ?? "pending");
    setComment(
      latestAnalysis?.notes ?? parcel?.terrainVerificationComment ?? "",
    );
    setSaveMessage(null);
    setSaveError(null);
  }
  ...
  async function setVerificationState(next: VerificationState) {
    ...
    try {
      const updated = await parcelService.updateVerification(
        parcel.id,
        next,
        comment.trim(),
      );
      ...
      setSaveMessage(
        next === "verified"
          ? "Vérification terrain enregistrée."
          : "Faux positif enregistré.",
      );
    } catch (error) {
      ...
      setSaveError(
        error instanceof Error
          ? error.message
          : "Impossible d’enregistrer la vérification terrain.",
      );
    }
  }

  const metrics = parcel ? computeParcelMetrics(parcel.boundary) : null;
  const severity = parcel ? computeParcelSeverity(parcel, thresholds) : null;
  ...
  const averageConfidence = confidenceValues.length
    ...
    : null;
  const geolocQuality: GeolocationQuality = images.some(
  ...
  async function saveComment() {
    ...
    setSaveMessage(null);
    try {
      const updated = await analysisService.updateNotes(
        latestAnalysis.id,
        comment.trim(),
      );
      const nextParcel = {
        ...parcel,
        analyses: (parcel.analyses ?? []).map((analysis) =>
      ...
      setSaveError(
        error instanceof Error
          ? error.message
          : "Impossible d’enregistrer le mémo.",
      );
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <Dialog
      open={open}
      onClose={onClose}
      title={parcel?.name ?? "Fiche complète de la parcelle"}
      maxWidthClassName="max-w-6xl"
    >
      {isLoading ? (
        <div className="flex min-h-[360px] items-center justify-center">
          <Spinner label="Chargement de la fiche complète..." />
        </div>
      ) : loadError ? (
              ...
              <div className="grid gap-4 lg:grid-cols-[230px_minmax(0,1fr)]">
                <aside className="rounded-2xl border border-slate-200 bg-white p-5">
                  ...
                  <p className="mt-4 text-center text-xs leading-relaxed text-slate-500">
                    Calculé sur {confidenceValues.length} image
                    {confidenceValues.length > 1 ? "s" : ""} avec score
                    disponible.
                  </p>
                </aside>
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <p className="text-sm font-bold text-slate-900">
                          Actions terrain — Vérification
                        </p>
                        <p className="mt-1 text-xs text-slate-600">
                          Qualifier rapidement le constat affiché sur la carte.
                        </p>
                      </div>
                      ...
                      <div className="text-xs">
                        ...
                        {!latestAnalysis && (
                          <span className="text-slate-500">
                            Une analyse est nécessaire pour enregistrer un mémo.
                          </span>
                        )}
                      </div>
                      <button
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <p className="mt-1 text-sm font-semibold text-slate-900">
                        {metrics.center.lat.toFixed(5)},{" "}
                        {metrics.center.long.toFixed(5)}
                      </p>
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-500">
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            {analysis.mission?.name ?? "Sans mission"} ·{" "}
                            {analysis.images?.length ?? 0} image
                            {(analysis.images?.length ?? 0) > 1 ? "s" : ""}
                          </p>
                        </div>
                    ...
                    >
                      ...
                      <div className="space-y-2 p-3 text-xs text-slate-600">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {GEOLOC_LABELS[image.geolocationQuality]}
                        </div>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  <h3 className="text-sm font-bold text-slate-900">
                    État de vérification terrain
                  </h3>
                  <p className="mt-2 text-sm text-slate-600">
                    {verification === "verified" &&
                      "Cette parcelle est enregistrée comme vérifiée sur le terrain."}
                    {verification === "false_positive" &&
                      "Le signal est enregistré comme faux positif confirmé."}
                    {verification === "pending" &&
                      "Aucune décision terrain n’a encore été enregistrée."}
                  </p>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  ...
                  <div className="mt-4 space-y-3 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500">Géolocalisation</span>
                      <span className="font-semibold text-slate-900">
                        {GEOLOC_LABELS[geolocQuality]}
                      </span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500">Confiance moyenne</span>
                      <span className="font-semibold text-slate-900">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500">Images traitées</span>
                      <span className="font-semibold text-slate-900">
                        {severity.processedImages}
                      </span>
                    </div>
                  </div>
                  {geolocQuality !== "precise" && (
                    <div className="mt-4 flex gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                      <AlertTriangle className="h-4 w-4 shrink-0" /> Une
                      vérification terrain est recommandée car la précision de
                      géolocalisation est dégradée.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  ...
                  <textarea
                    ...
                    onChange={(event) => setComment(event.target.value)}
                    rows={8}
                    placeholder="Ajoutez les observations terrain, symptômes, traitements ou décisions..."
                    className="mt-3 w-full resize-y rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-[#6E8B3D] focus:ring-2 focus:ring-[#6E8B3D]/15"
                  />
                  <div className="mt-3 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      onClick={saveComment}
                      disabled={!latestAnalysis || isSaving}
                      className="rounded-xl bg-[#244B32] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#356A46] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                  ...
                  <div className="mt-3 space-y-3">
                    {analyses.filter((analysis) => analysis.notes).length ? (
                      analyses
                        .filter((analysis) => analysis.notes)
                        .map((analysis) => (
                          <div
                            key={analysis.id}
