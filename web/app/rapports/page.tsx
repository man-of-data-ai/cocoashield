...
import { CalendarDays, ChevronRight, FileDown, FileText, Filter, Search, Sprout, CheckCircle2, Clock3, AlertTriangle } from "lucide-react";
...
import { exportPeriodicSummaryPdf } from "@/lib/periodic-summary-pdf";
...
function ReportsPageContent() {
  ...
  function exportPeriodicSummary() {
    const days = period === "all" ? 30 : Number(period);
    const now = referenceNow;
    const currentStart = now - days * 86400000;
    const previousStart = now - days * 2 * 86400000;
    const current = rows.filter((row) => row.date.getTime() >= currentStart && row.date.getTime() <= now);
    const previous = rows.filter((row) => row.date.getTime() >= previousStart && row.date.getTime() < currentStart);
    exportPeriodicSummaryPdf({
      current: current.map((row) => ({ parcel: row.parcel, analysis: row.analysis, date: row.date })),
      previous: previous.map((row) => ({ parcel: row.parcel, analysis: row.analysis, date: row.date })),
  ...
  return <AppShell title="Rapports">
    ...
    <section className="rounded-[26px] border border-[#E2E9DE] bg-white p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[1.5fr_1fr_1fr_1fr]">
        ...
        <ModernSelect value={period} onChange={setPeriod} options={[{ value: "all", label: "Toutes les dates" }, { value: "7", label: "7 derniers jours" }, { value: "30", label: "30 derniers jours" }, { value: "90", label: "3 derniers mois" }, { value: "365", label: "12 derniers mois" }]} />
      </div>
    </section>

    <section className="mt-5 overflow-hidden rounded-[26px] border border-[#E2E9DE] bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#EDF0EA] p-5"><div><h2 className="font-bold text-slate-900">Rapports de suivi</h2><p className="mt-1 text-xs text-slate-400">{filtered.length} résultat{filtered.length > 1 ? "s" : ""}</p></div><button type="button" onClick={exportPeriodicSummary} disabled={rows.length === 0} className="inline-flex items-center gap-2 rounded-xl bg-[#244B32] px-4 py-2.5 text-xs font-bold text-white hover:bg-[#356A46] disabled:opacity-50"><FileDown className="h-4 w-4"/>Exporter la synthèse périodique</button></div>
      {loading ? <div className="flex justify-center py-16"><Spinner label="Chargement des rapports..." /></div> : filtered.length === 0 ? <div className="py-16 text-center"><Sprout className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 text-sm font-semibold text-slate-600">Aucun rapport pour ces critères</p><p className="mt-1 text-xs text-slate-400">Modifiez les filtres ou réalisez une nouvelle analyse terrain.</p></div> : <div className="divide-y divide-[#EDF0EA]">{filtered.map((row) => {
        ...
        return <article key={row.id} className={`group grid gap-4 p-5 transition hover:bg-[#FBFCFA] lg:grid-cols-[1.2fr_.9fr_.8fr_1.4fr_auto] lg:items-center ${focusedAnalysisId === row.id ? "bg-[#F2F7EE] ring-1 ring-inset ring-[#BFD2B4]" : ""}`}>
          ...
          <Link href={`/rapports/${row.analysis.id}`} className="flex h-10 w-10 items-center justify-center rounded-2xl border border-[#E0E7DC] text-slate-400 transition group-hover:border-[#B8CEAD] group-hover:bg-white group-hover:text-[#4D713B]" aria-label="Voir le rapport"><ChevronRight className="h-4 w-4" /></Link>
        </article>;
      })}</div>}
    </section>
  </AppShell>;
