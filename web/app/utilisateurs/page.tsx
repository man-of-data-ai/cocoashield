...
import { Search, ShieldCheck, UserRound, UsersRound, Pencil, Building2, CheckCircle2, Plus, Trash2, AlertTriangle } from "lucide-react";
...
const ROLE_OPTIONS = [
  ...
  { value: "agronome_terrain", label: "Agronome Terrain", description: "Parcelles, missions et observations", badge: "TERRAIN" },
];
...
export default function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({ name: "", email: "", password: "CocoaDemo2026!", role: "agronome_terrain", cooperative: "" });
  const [pendingRole, setPendingRole] = useState<{ user: AppUser; nextRole: UserRole } | null>(null);
  const [pendingDelete, setPendingDelete] = useState<AppUser | null>(null);
  const [deleteConfirmation, setDeleteConfirmation] = useState("");

  useEffect(() => {
    userService.list().then(setUsers).catch((err) => setError(err instanceof Error ? err.message : "Impossible de charger les utilisateurs.")).finally(() => setLoading(false));
  }, []);
  ...
  async function updateRole(user: AppUser, nextRole: string) {
    ...
    setError(null);
    try {
      const updated = await userService.update(user.id, { role: nextRole as UserRole });
      setUsers((current) => current.map((item) => item.id === updated.id ? updated : item));
    } catch (err) {
      setError(err instanceof Error ? err.message : "La mise à jour du rôle a échoué.");
    } finally {
      setSavingId(null);
    }
  }
  ...
  async function createUser() {
    ...
    setError(null);
    try {
      const created = await userService.create({ email: newUser.email.trim(), username: newUser.name.trim().replace(/\s+/g, "."), password: newUser.password, role: newUser.role as UserRole, cooperative: newUser.cooperative.trim() });
      setUsers((current) => [created, ...current]);
      setCreateOpen(false);
      setNewUser({ name: "", email: "", password: "CocoaDemo2026!", role: "agronome_terrain", cooperative: "" });
    } catch (err) {
    ...
    <AppShell title="Utilisateurs" description="Gérez les profils et les responsabilités au sein de Cocoashield." headerActions={<button type="button" onClick={() => setCreateOpen(true)} className="inline-flex items-center gap-2 rounded-2xl bg-[#244B32] px-4 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-[#356A46]"><Plus className="h-4 w-4" /><span className="hidden sm:inline">Ajouter utilisateur</span></button>}>
            ...
            <table className="w-full min-w-[900px] text-left">
              ...
              <tbody>
                {filtered.map((user) => <tr key={user.id} className="border-t border-[#EDF0EA] transition hover:bg-[#FBFCFA]">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-600">{user.cooperative || "Cocoashield"}</td>
                  <td className="px-5 py-4"><span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" />{user.status === "active" ? "Actif" : "Inactif"}</span></td>
                  <td className="px-5 py-4"><div className="flex items-center gap-2"><Pencil className="h-4 w-4 text-slate-300" /><ModernSelect value={user.role} onChange={(value) => value !== user.role && setPendingRole({ user, nextRole: value as UserRole })} options={ROLE_OPTIONS} disabled={savingId === user.id} className="w-56" /><button type="button" onClick={() => { setPendingDelete(user); setDeleteConfirmation(""); }} disabled={savingId === user.id} className="flex h-10 w-10 items-center justify-center rounded-xl border border-red-100 bg-red-50 text-red-500 transition hover:bg-red-100 disabled:opacity-40" aria-label={`Supprimer ${user.name}`}><Trash2 className="h-4 w-4" /></button></div></td>
                </tr>)}
              </tbody>
            </table>
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title="Ajouter un utilisateur">
        <div className="space-y-4">
          ...
          <label className="block text-xs font-bold text-slate-600">Rôle<ModernSelect className="mt-1.5" value={newUser.role} onChange={(value) => setNewUser((current) => ({ ...current, role: value }))} options={ROLE_OPTIONS} /></label>
          <label className="block text-xs font-bold text-slate-600">Organisation<input value={newUser.cooperative} onChange={(event) => setNewUser((current) => ({ ...current, cooperative: event.target.value }))} placeholder="COOP-CA Soubré" className="mt-1.5 w-full rounded-2xl border border-[#DDE6D9] bg-[#FAFCF8] px-3.5 py-2.5 text-sm font-normal outline-none focus:border-[#9BBC89]" /></label>
          <label className="block text-xs font-bold text-slate-600">Mot de passe temporaire<input type="text" value={newUser.password} onChange={(event) => setNewUser((current) => ({ ...current, password: event.target.value }))} className="mt-1.5 w-full rounded-2xl border border-[#DDE6D9] bg-[#FAFCF8] px-3.5 py-2.5 text-sm font-normal outline-none focus:border-[#9BBC89]" /></label>
          <div className="flex justify-end gap-2 pt-2"><button type="button" onClick={() => setCreateOpen(false)} className="rounded-2xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600">Annuler</button><button type="button" onClick={() => void createUser()} disabled={createLoading} className="rounded-2xl bg-[#244B32] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{createLoading ? "Création..." : "Créer le compte"}</button></div>
        </div>
      </Dialog>

      <Dialog open={pendingRole !== null} onClose={() => setPendingRole(null)} title="Confirmer le changement de rôle">
        {pendingRole && <div className="space-y-4">
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><div className="flex items-start gap-3"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0"/><div><p className="font-bold">Vérifiez cette modification avant de continuer.</p><p className="mt-1 text-xs leading-5">Les droits d’accès de cet utilisateur seront mis à jour immédiatement.</p></div></div></div>
          <div className="rounded-2xl bg-[#F7F9F5] p-4"><p className="text-sm font-bold text-slate-900">{pendingRole.user.name}</p><p className="mt-1 text-xs text-slate-500">{pendingRole.user.email}</p><div className="mt-4 grid grid-cols-2 gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Ancien rôle</p><p className="mt-1 text-sm font-semibold text-slate-700">{roleLabel(pendingRole.user.role)}</p></div><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Nouveau rôle</p><p className="mt-1 text-sm font-semibold text-[#244B32]">{roleLabel(pendingRole.nextRole)}</p></div></div></div>
          <div className="flex justify-end gap-2"><button type="button" onClick={() => setPendingRole(null)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600">Annuler</button><button type="button" onClick={() => void confirmRoleChange()} disabled={savingId === pendingRole.user.id} className="rounded-xl bg-[#244B32] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">Confirmer le rôle</button></div>
        </div>}
      </Dialog>
