...
import { Activity, Camera, MapPinned, Route, Ruler, ScanSearch } from "lucide-react";
...
export default function ParcelDetailPage() {
  ...
  useEffect(()=>{
    let mounted=true;
    parcelService.getParcel(params.id)
      .then((d)=>mounted&&setParcel(d))
      .catch((e)=>mounted&&setError(e instanceof ApiError?e.message:"Impossible de charger la parcelle."))
  ...
  const relatedMissions = useMemo(() => {
    ...
    for (const analysis of parcel.analyses) if (analysis.mission) seen.set(analysis.mission.id, analysis.mission);
    return Array.from(seen.values()).sort((a,b)=>new Date(b.missionDate??b.createdAt).getTime()-new Date(a.missionDate??a.createdAt).getTime());
  }, [parcel]);
  ...
  return <AppShell title={parcel.name} headerActions={<StatusBadge status={parcel.status}/>}> 
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
      ...
      <button onClick={()=>setAnalysisOpen(true)} className="inline-flex items-center gap-2 rounded-2xl bg-[#244B32] px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#356A46]"><ScanSearch className="h-4 w-4"/>Lancer une analyse</button>
    </div>
