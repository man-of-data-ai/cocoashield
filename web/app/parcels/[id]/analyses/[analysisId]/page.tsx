...
import AnalysisImageExplorer from "@/components/analyses/AnalysisImageExplorer";
...
function AnalysisDetailPageContent() {
  ...
  useEffect(()=>{ if(!analysis || analysis.status==="completed") return; const timer=window.setInterval(()=>{ analysisService.getAnalysis(params.analysisId).then((fresh)=>{ setAnalysis(fresh); if(fresh.status==="completed") window.clearInterval(timer); }).catch(()=>undefined); },3000); return()=>window.clearInterval(timer); },[analysis?.status,params.analysisId]);
  ...
  const riskZones=useMemo(()=>analysis?.parcel?riskZonesFromAnalysis(analysis.parcel,analysis):[],[analysis]);
  useEffect(()=>{if(!selectedRiskZone||!analysis)return;const located=analysis.images.filter(i=>i.latitude!==null&&i.longitude!==null);if(!located.length)return;const dist=(i:typeof located[number])=>{const dy=(i.latitude!-selectedRiskZone.latitude)*111320;const dx=(i.longitude!-selectedRiskZone.longitude)*111320*Math.cos(selectedRiskZone.latitude*Math.PI/180);return dx*dx+dy*dy};const nearest=[...located].sort((a,b)=>dist(a)-dist(b))[0];if(nearest)setSelected(nearest.id);},[selectedRiskZone,analysis]);
