...
import type {
  MapBasemap,
  MapLayersState,
} from "@/components/map/ParcelsOverviewMap";
...
const ParcelsOverviewMap = nextDynamic(
  () => import("@/components/map/ParcelsOverviewMap"),
  {
    ssr: false,
    loading: () => (
...
function freshnessLabel(
  entries: ReturnType<typeof collectImageEntries>,
): string {
  if (entries.length === 0) return "—";
  const latest = Math.max(
    ...entries.map((entry) => new Date(entry.image.createdAt).getTime()),
  );
...
function MapDashboardPageContent() {
  ...
  const [fullDetailsParcel, setFullDetailsParcel] = useState<Parcel | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (!isMounted) return;
      const [parcelsResult, missionsResult, droneProfilesResult] = results;
      if (parcelsResult.status === "fulfilled") setParcels(parcelsResult.value);
      else
        setError(
          parcelsResult.reason instanceof ApiError
      ...
      if (missionsResult.status === "fulfilled")
        ...
        console.warn(
          ...
          missionsResult.reason,
        );
      if (droneProfilesResult.status === "fulfilled")
        setDroneProfiles(droneProfilesResult.value);
      setIsLoading(false);
    });
    return () => {
      isMounted = false;
  ...
  async function openFullDetails(parcel: Parcel) {
    ...
    try {
      ...
      setParcels((current) =>
        current.map((item) =>
          item.id === detailedParcel.id ? detailedParcel : item,
        ),
      );
    } catch (err) {
      setFullDetailsError(
        err instanceof ApiError
          ? err.message
          : "Impossible de charger la fiche complète de la parcelle.",
      );
    } finally {
  ...
  const allImageEntries = useMemo(
    () => collectImageEntries(parcels),
    [parcels],
  );
  const dataFilteredEntries = useMemo(
    () => applyDataFilters(allImageEntries, filters),
    [allImageEntries, filters],
  );
  const parcelAggregates = useMemo(
    () => aggregateByParcel(dataFilteredEntries, thresholds),
    [dataFilteredEntries, thresholds],
  );

  const aggregatesWithEmptyParcels = useMemo(() => {
    const byId = new Map(
      parcelAggregates.map((aggregate) => [aggregate.parcel.id, aggregate]),
    );
    ...
    return result;
  }, [parcelAggregates, parcels, filters, thresholds]);

  const visibleAggregates = useMemo(
    () =>
      applySeverityAndSearchFilters(aggregatesWithEmptyParcels, filters).sort(
        (a, b) => severityRank(b.level) - severityRank(a.level),
      ),
    [aggregatesWithEmptyParcels, filters],
  );
  const visibleParcelIds = useMemo(
    () => new Set(visibleAggregates.map((aggregate) => aggregate.parcel.id)),
    [visibleAggregates],
  );
  const visibleImageEntries = useMemo(
    () =>
      dataFilteredEntries.filter((entry) =>
        visibleParcelIds.has(entry.parcel.id),
      ),
    [dataFilteredEntries, visibleParcelIds],
  );
  const selectedParcel = useMemo(
    () =>
      visibleAggregates.find(
        (aggregate) => aggregate.parcel.id === selectedParcelId,
      )?.parcel ?? null,
    [visibleAggregates, selectedParcelId],
  );
  ...
  const visibleRiskZones = useMemo(() => {
    const analyses = new Map<
      ...
      {
        parcel: Parcel;
        analysis: (typeof visibleImageEntries)[number]["analysis"];
        imageIds: Set<string>;
      }
    >();
    for (const entry of visibleImageEntries) {
      if (requestedAnalysisId && entry.analysis.id !== requestedAnalysisId)
        continue;
      const current = analyses.get(entry.analysis.id) ?? {
        parcel: entry.parcel,
    ...
    return Array.from(analyses.values()).flatMap(
      ({ parcel, analysis, imageIds }) =>
        riskZonesFromAnalysis(parcel, analysis, thresholds, imageIds),
    );
  }, [visibleImageEntries, requestedAnalysisId, thresholds]);
  const activeSeverityLevels = filters.activeLevels.filter(
    (level): level is Severity => level !== "inconnu",
  );
  const requestedAnalysis = requestedAnalysisId
    ? (parcels
        .flatMap((parcel) =>
          (parcel.analyses ?? []).map((analysis) => ({ parcel, analysis })),
        )
        .find((entry) => entry.analysis.id === requestedAnalysisId) ?? null)
    : null;

  const kpis = useMemo(() => {
    const missionIds = new Set(
      visibleImageEntries
        .map((entry) => entry.analysis.missionId)
        .filter((id): id is string => Boolean(id)),
    );
    const infectedSurfaceSquareMeters = visibleAggregates.reduce(
      (sum, aggregate) => {
        const area = computeParcelMetrics(
  ...
  return (
    <AppShell title="Carte">
      {requestedAnalysisId && requestedParcelId && (
        <div className="mb-4">
          <Alert variant="info">
            Vue ouverte depuis une analyse : la parcelle concernée est
            présélectionnée et les zones à risque affichées correspondent à
            cette analyse.
          </Alert>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <Link
            href="/parcels"
            className="mt-3 inline-block rounded-xl bg-[#244B32] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#356A46]"
          >
            Créer une parcelle
          </Link>
        </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)_330px] xl:items-start">
            ...
            <div className="overflow-hidden rounded-[24px] border border-[#DFE7DB] bg-white shadow-sm">
              <MapToolbar
                ...
                exportData={{
                  parcels: visibleAggregates.map((aggregate) => ({
                    id: aggregate.parcel.id,
                    name: aggregate.parcel.name,
                    boundary: aggregate.parcel.boundary,
                  })),
                  riskZones: visibleRiskZones.filter((zone) =>
                    activeSeverityLevels.includes(zone.level),
                  ),
                }}
                exportContext={{
                  title: requestedAnalysis
                    ? `Analyse · ${requestedAnalysis.parcel.name}`
                    : "Cocoashield — Cartographie phytosanitaire",
                  subtitle: requestedAnalysis
                    ? `Analyse du ${new Date(requestedAnalysis.analysis.completedAt ?? requestedAnalysis.analysis.createdAt).toLocaleString("fr-FR", { dateStyle: "medium", timeStyle: "short" })}`
                    : `${visibleAggregates.length} parcelle${visibleAggregates.length > 1 ? "s" : ""} affichée${visibleAggregates.length > 1 ? "s" : ""}`,
                  details: requestedAnalysis
                    ? [
                        `Infection : ${(requestedAnalysis.analysis.infectionPercentage ?? 0).toFixed(1)} %`,
                        `Sévérité : ${requestedAnalysis.analysis.severityLevel ?? "En attente"}`,
                        `Zones à risque affichées : ${visibleRiskZones.length}`,
                      ]
                    : [
                        `Missions : ${kpis.missionsCount}`,
                        `Zones affichées : ${visibleRiskZones.length}`,
                        `Surface infectée estimée : ${(kpis.infectedSurfaceSquareMeters / 10_000).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} ha`,
                      ],
                }}
              />
              <div className="relative h-[640px] min-h-[520px] overflow-hidden bg-slate-100 xl:h-[720px]">
                <ParcelsOverviewMap
                  parcels={visibleAggregates}
                  riskZones={visibleRiskZones}
