...
const DEFAULT_LAYERS: MapLayersState = { zones: true, heatmap: true, points: true };
...
function MapDashboardPageContent() {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    analysisService.getAnalysis(requestedAnalysisId)
      .then((analysis) => {
        if (!mounted) return;
        setFocusedAnalysis(analysis);
        setSelectedParcelId(analysis.parcelId);
      })
      .catch((err) => {
        if (mounted) setError(err instanceof ApiError ? err.message : "Impossible de charger l’analyse demandée.");
      });
  ...
  const aggregatesWithEmptyParcels = useMemo(() => {
    ...
    const dataFilterRestrictive =
      ...
      filters.vectors.length < 3 ||
      filters.droneProfileIds.length > 0 ||
      filters.geolocQualities.length < 7 ||
      filters.period !== "all";
  ...
  const requestedAnalysis = useMemo(() => {
    if (!requestedAnalysisId) return null;
    if (focusedAnalysis) {
      const parcel = focusedAnalysis.parcel ?? parcels.find((item) => item.id === focusedAnalysis.parcelId);
      return parcel ? { parcel, analysis: focusedAnalysis } : null;
    }
    return parcels.flatMap((parcel) => (parcel.analyses ?? []).map((analysis) => ({ parcel, analysis }))).find((entry) => entry.analysis.id === requestedAnalysisId) ?? null;
  }, [requestedAnalysisId, focusedAnalysis, parcels]);

  const visibleRiskZones = useMemo(() => {
    if (requestedAnalysis) return riskZonesFromAnalysis(requestedAnalysis.parcel, requestedAnalysis.analysis);
  ...
  }, [visibleImageEntries, requestedAnalysis]);
  const activeSeverityLevels = filters.activeLevels.filter((level): level is Severity => level !== "inconnu");
  const selectedRequestedAnalysis = requestedAnalysis && selectedParcel && requestedAnalysis.parcel.id === selectedParcel.id ? requestedAnalysis.analysis : null;
