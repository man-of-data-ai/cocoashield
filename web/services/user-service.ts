...
export const userService = {
  async create(input: { email: string; username: string; password: string; role: UserRole; cooperative?: string }): Promise<AppUser> {
    ...
    return this.update(created.user.id, { role: input.role, cooperative: input.cooperative || null, status: "active" });
  },
  ...
  update(id: string, input: { role?: UserRole; cooperative?: string | null; status?: UserStatus }): Promise<AppUser> {
    return apiRequest<AppUser>(`/v1/users/${id}/profile`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) });
  },
  remove(id: string): Promise<{ id: string; deleted: boolean }> {
    return apiRequest<{ id: string; deleted: boolean }>(`/v1/users/${id}`, { method: "DELETE" });
  },
};
