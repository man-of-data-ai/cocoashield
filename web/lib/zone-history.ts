...
export type AnalysisComparisonSnapshot = {
  parcel: Parcel;
  analysis: import("@/types/parcel").Analysis;
  images: AnalysisImage[];
  level: SeverityLevel;
  infectionRate: number;
  processedImages: number;
...
export function buildAnalysisComparisonSnapshot(
  parcel: Parcel,
  analysis: import("@/types/parcel").Analysis
  ...
  let area = 0;
  try {
    area = computeParcelMetrics(parcel.boundary).areaSquareMeters;
  } catch {
    area = 0;
  }
  return {
    ...
    processedImages: severity.processedImages,
    infectedImages: severity.infectedImages,
    infectedAreaSquareMeters: area * severity.infectionRate,
    observedAreaSquareMeters: area,
  };
}
