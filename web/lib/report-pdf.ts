...
function addFooter(doc: jsPDF, pageNumber: number) {
  ...
  doc.text("Cocoashield — Rapport d’analyse phytosanitaire", PAGE.margin, PAGE.height - 7);
  doc.text(`Page ${pageNumber}`, PAGE.width - PAGE.margin, PAGE.height - 7, { align: "right" });
}
...
function ensureSpace(doc: jsPDF, y: number, required: number, pageNumberRef: { value: number }): number {
...
function addWrappedText(doc: jsPDF, text: string, y: number, pageNumberRef: { value: number }, options?: { bullet?: boolean; maxWidth?: number; lineHeight?: number }): number {
  const maxWidth = options?.maxWidth ?? PAGE.width - PAGE.margin * 2 - (options?.bullet ? 6 : 0);
  const lineHeight = options?.lineHeight ?? 5;
  const lines = doc.splitTextToSize(safeText(text), maxWidth) as string[];
  y = ensureSpace(doc, y, Math.max(8, lines.length * lineHeight), pageNumberRef);
  doc.setFontSize(9.5);
...
function drawKpi(doc: jsPDF, x: number, y: number, width: number, label: string, value: string) {
...
function drawObservationChart(doc: jsPDF, data: AnalysisReportData, y: number): number {
  ...
  doc.roundedRect(PAGE.margin + 32, y - 4, (width - 32) * healthyRatio, 4, 2, 2, "F");
  y += 9;
  doc.text(`Infectées  ${data.infectedImages}`, PAGE.margin, y);
  doc.setFillColor(245, 235, 231);
  doc.roundedRect(PAGE.margin + 32, y - 4, width - 32, 4, 2, 2, "F");
  doc.setFillColor(184, 69, 51);
  doc.roundedRect(PAGE.margin + 32, y - 4, (width - 32) * infectedRatio, 4, 2, 2, "F");
  return y + 8;
}
...
export function exportAnalysisReportPdf(data: AnalysisReportData): void {
  ...
  doc.text(`${safeText(data.parcelName)} · ${safeText(data.missionName)}`, PAGE.margin, 33);

  let y = 51;
  doc.setFontSize(9);
  doc.setTextColor(100, 116, 139);
  doc.text(`Analyse : ${formatDate(data.analysisDate)}`, PAGE.margin, y);
  doc.text(`Rapport : ${formatDate(data.reportDate)}`, PAGE.width - PAGE.margin, y, { align: "right" });
  y += 8;

  const gap = 3;
  const kpiWidth = (PAGE.width - PAGE.margin * 2 - gap * 3) / 4;
  drawKpi(doc, PAGE.margin, y, kpiWidth, "Infection", `${data.infectionPercentage.toFixed(1)} %`);
  drawKpi(doc, PAGE.margin + (kpiWidth + gap), y, kpiWidth, "Sévérité", data.severityLabel);
  drawKpi(doc, PAGE.margin + (kpiWidth + gap) * 2, y, kpiWidth, "Images", String(data.totalImages));
  drawKpi(doc, PAGE.margin + (kpiWidth + gap) * 3, y, kpiWidth, "Zones", String(data.affectedZoneCount));
  y += 31;

  y = sectionTitle(doc, "Résumé de l’analyse", y);
  const summary = data.processedImages > 0
  ...
  if (data.notes) {
    y = addWrappedText(doc, `Observation terrain : ${data.notes}`, y, pageNumber);
  }

  y = ensureSpace(doc, y + 3, 35, pageNumber);
  y = sectionTitle(doc, "Répartition des observations", y);
  y = drawObservationChart(doc, data, y + 2);
  y = addWrappedText(doc, `${data.preciseImages} image${data.preciseImages > 1 ? "s" : ""} dispose${data.preciseImages > 1 ? "nt" : ""} d’une géolocalisation précise. ${data.locatedInfectedImages} observation${data.locatedInfectedImages > 1 ? "s" : ""} infectée${data.locatedInfectedImages > 1 ? "s" : ""} est/sont localisable${data.locatedInfectedImages > 1 ? "s" : ""}.`, y, pageNumber);
  ...
  for (const line of areaLines) y = addWrappedText(doc, line, y, pageNumber, { bullet: true });

  if (data.zones.length > 0) {
    y = ensureSpace(doc, y + 3, 30, pageNumber);
    y = sectionTitle(doc, "Zones à risque", y);
    doc.setFillColor(244, 247, 242);
    doc.roundedRect(PAGE.margin, y - 2, PAGE.width - PAGE.margin * 2, 8, 2, 2, "F");
    doc.setFontSize(8);
    doc.setTextColor(71, 85, 105);
    doc.setFont("helvetica", "bold");
  ...
  for (const insight of data.insights) y = addWrappedText(doc, insight, y, pageNumber, { bullet: true });

  y = ensureSpace(doc, y + 3, 28, pageNumber);
  y = sectionTitle(doc, "Recommandations de suivi", y);
  for (const recommendation of data.recommendations) y = addWrappedText(doc, recommendation, y, pageNumber, { bullet: true });
  ...
  const notice = doc.splitTextToSize("Ce rapport constitue un outil d’aide à la surveillance phytosanitaire. Les décisions d’intervention doivent être confirmées par une observation terrain lorsque cela est nécessaire.", PAGE.width - PAGE.margin * 2 - 8) as string[];
  doc.text(notice, PAGE.margin + 4, y + 6);

  addFooter(doc, pageNumber.value);
  doc.save(`rapport-cocoashield-${data.parcelName.toLowerCase().replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "analyse"}-${timestampForFilename()}.pdf`);
}
