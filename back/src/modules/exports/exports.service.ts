  ...
  private async prepare(userId: string, dto: CreateExportDto): Promise<PreparedExport> {
    ...
    const selected = parcels.filter((parcel) => {
      ...
      if (dto.scope === ExportScope.ZONE) {
        if (parcel.id !== dto.scopeId) return false;
        scopeLabel = parcel.name;
    ...
    const features = selected.map((parcel): ExportFeature => {
      ...
      return {
        ...
        properties: {
          ...
          infection_rate:
            processed.length > 0
              ? Number(((infected.length / processed.length) * 100).toFixed(2))
              : 0,
        ...
        return {
          ...
          data: createSimplePdf('Cocoashield - Export phytosanitaire', [
            ...
            ...prepared.features.flatMap((feature) => [
              ...
              `Taux infection : ${feature.properties.infection_rate}% — Images : ${feature.properties.image_count} — Vérification : ${feature.properties.verification}`,
            ]),
